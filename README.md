...
La position evolue a partir de la vitesse selon :

\[
\dot{x} = v_x,\qquad \dot{y} = v_y,\qquad \dot{z} = v_z
\]

L'acceleration du lanceur est calculee a partir de la poussee, de la trainee et de la gravite :
...
avec :
- \(m\) la masse instantanee du lanceur ;
- \(\mathbf{T}_{world}\) la poussee exprimee dans le repere inertiel ;
- \(\mathbf{D}\) la force de trainee ;
- \(g\) l'acceleration de la pesanteur.

La poussee est appliquee suivant l'axe du lanceur puis projetee dans le repere inertiel :

\[
\mathbf{T}_{world} = R(\psi,\theta,\phi)\begin{bmatrix}0\\0\\T\end{bmatrix}
\]

### 2. Atmosphere et trainee aerodynamique
...
Le vent est introduit a travers la vitesse relative entre le lanceur et l'air :
...
La trainee est alors calculee par :
...
ou :
- \(C_d\) est le coefficient de trainee ;
- \(S\) est la surface de reference.

### 3. Equations de rotation

La dynamique rotationnelle repose sur le vecteur vitesse angulaire :

\[
\boldsymbol{\omega} = \begin{bmatrix}p\\q\\r\end{bmatrix}
\]

L'equation d'Euler du solide rigide utilisee dans le modele est :
...
- \(\boldsymbol{\tau}\) le couple total applique ;
- \(c_w\boldsymbol{\omega}\) un terme d'amortissement simplifie.

Le couple total comprend la commande et le couple perturbateur du vent :
...
Le couple de vent est calcule a partir d'un bras de levier aerodynamique :
...
L'attitude du lanceur est decrite par les angles d'Euler \((\phi,\theta,\psi)\), correspondant respectivement au roulis, au tangage et au lacet. Leur evolution est obtenue a partir des vitesses angulaires :
...
Dans l'implementation retenue, le terme derive est applique sur une mesure filtree afin de limiter les pics de commande. La commande angulaire prend donc la forme :

\[
M = k_p e + k_i I - k_d \dot{\theta}_{filtre}
\]

Puis elle est limitee par saturation :
...
Le modele est integre numeriquement par un schema d'Euler explicite :

\[
x_{k+1} = x_k + \dot{x}_k \Delta t
\]

Ce choix est suffisant ici pour etudier l'effet du correcteur PID, du vent et des gains de commande sur la dynamique globale du lanceur.
