...
%   - out/blender/import_modele_vega_PID_ok_blender_cinematic.py
    ...
    export_wind_diagnostics_plot(fullfile(paths.images_dir, 'modele_vega_PID_ok_wind.png'), data, cfg);
    export_wind_diagnostics_plot(fullfile(paths.docs_figures_dir, 'figure_diagnostic_vent_modele_vega_PID_ok.png'), data, cfg);
    ...
    fprintf('Diagnostic vent: %s\n', fullfile(paths.images_dir, 'modele_vega_PID_ok_wind.png'));
    if cfg.export_video
...
function cfg = build_config(mode_name, pid_override)
    ...
    cfg.wind_base_x = 22.0;
    cfg.wind_base_y = 7.5;
    cfg.wind_altitude_scale = 9000.0;
    cfg.wind_shear_altitude = 18000.0;
    cfg.wind_vertical_amplitude = 1.2;
    cfg.wind_vertical_center = 12000.0;
    cfg.wind_vertical_sigma = 5000.0;
    cfg.wind_gust_x = 10.0;
    cfg.wind_gust_y = -5.5;
    cfg.wind_gust_t0 = 8.0;
    cfg.wind_gust_sigma = 2.0;
    cfg.wind_normal_force_coeff = 0.08;
    cfg.wind_cp_offset_fraction = 0.18;
    cfg.pid_phi = struct('kp', 6.4e5, 'ki', 2.4e4, 'kd', 2.2e5);
    cfg.pid_theta = struct('kp', 2.2e6, 'ki', 6.0e4, 'kd', 2.2e5);
    cfg.pid_psi = struct('kp', 5.2e5, 'ki', 1.8e4, 'kd', 2.0e5);
    cfg.pid_integral_limit = deg2rad(12);
    cfg.pid_ref_filter_tau = 1.0;
    cfg.pid_derivative_filter_tau = 0.20;
...
function paths = build_paths()
    ...
    paths.docs_figures_dir = fullfile(project_dir, 'docs', 'figures');
end
...
function ensure_output_dirs(paths)
    ...
    if ~exist(paths.docs_figures_dir, 'dir')
        mkdir(paths.docs_figures_dir);
    end
end
...
function data = simulate_open_loop_3d(cfg)
    ...
    wind_x = zeros(1, N); wind_y = zeros(1, N); wind_z = zeros(1, N);
    tau_wind_x = zeros(1, N); tau_wind_y = zeros(1, N); tau_wind_z = zeros(1, N);
    ...
    for k = 2:N
        ...
        wind_air = wind_velocity_profile(max(z(k - 1), 0), tk, cfg);
        wind_x(k) = wind_air(1);
        wind_y(k) = wind_air(2);
        wind_z(k) = wind_air(3);
        vel_rel = vel - wind_air;
        drag = 0.5 * rho * Cd * reference_area * norm(vel_rel) * vel_rel;
        acc = (thrust_world - drag) / current_mass - [0; 0; g];

        omega = [p(k - 1); q(k - 1); r(k - 1)];
        q_dyn = 0.5 * rho * norm(vel_rel)^2;
        tau_wind = aero_wind_moment(vel_rel, R, q_dyn, reference_area, rocket, cfg);
        tau_wind_x(k) = tau_wind(1);
        tau_wind_y(k) = tau_wind(2);
        tau_wind_z(k) = tau_wind(3);
        tau = [Mx_ref; My_ref; Mz_ref] + tau_wind;
    ...
    data.wind_x = wind_x(1:impact_idx);
    data.wind_y = wind_y(1:impact_idx);
    data.wind_z = wind_z(1:impact_idx);
    data.tau_wind_x = tau_wind_x(1:impact_idx);
    data.tau_wind_y = tau_wind_y(1:impact_idx);
    data.tau_wind_z = tau_wind_z(1:impact_idx);
...
function wind_air = wind_velocity_profile(altitude_m, tk, cfg)
    if ~cfg.wind_enabled
        wind_air = zeros(3, 1);
        return;
    end

    base_x = cfg.wind_base_x * (1 - exp(-altitude_m / max(cfg.wind_altitude_scale, eps)));
    shear_y = cfg.wind_base_y * tanh(altitude_m / max(cfg.wind_shear_altitude, eps));
    vertical = cfg.wind_vertical_amplitude * exp(-((altitude_m - cfg.wind_vertical_center) / max(cfg.wind_vertical_sigma, eps))^2);
    gust_gain = exp(-((tk - cfg.wind_gust_t0) / max(cfg.wind_gust_sigma, eps))^2);

    wind_air = [ ...
        base_x + cfg.wind_gust_x * gust_gain; ...
        shear_y + cfg.wind_gust_y * gust_gain; ...
        vertical];
end

function tau_wind = aero_wind_moment(vel_rel_world, R, q_dyn, reference_area, rocket, cfg)
    if ~cfg.wind_enabled || q_dyn <= 0
        tau_wind = zeros(3, 1);
        return;
    end

    vel_rel_body = R' * vel_rel_world;
    lateral_body = [vel_rel_body(1); vel_rel_body(2); 0];
    lateral_speed = norm(lateral_body);
    if lateral_speed < 1e-9
        tau_wind = zeros(3, 1);
        return;
    end

    side_force_body = -q_dyn * cfg.wind_normal_force_coeff * reference_area * (lateral_body / lateral_speed);
    cp_offset_body = [0; 0; -cfg.wind_cp_offset_fraction * rocket.body_length];
    tau_wind = cross(cp_offset_body, side_force_body);
end
...
function export_wind_diagnostics_plot(output_path, data, cfg)
    has_wind = isfield(data, 'wind_x') && isfield(data, 'wind_y') && isfield(data, 'wind_z');
    has_tau = isfield(data, 'tau_wind_x') && isfield(data, 'tau_wind_y') && isfield(data, 'tau_wind_z');
    if ~has_wind && ~has_tau
        return;
    end

    fig = figure('Visible', 'off', 'Color', 'w', 'Position', [140 100 1180 760]);
    tl = tiledlayout(fig, 2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');

    ax1 = nexttile(tl, 1);
    hold(ax1, 'on'); grid(ax1, 'on'); box(ax1, 'on');
    if has_wind
        plot(ax1, data.t, data.wind_x, 'LineWidth', 1.5, 'Color', [0.12 0.45 0.85]);
        plot(ax1, data.t, data.wind_y, 'LineWidth', 1.5, 'Color', [0.85 0.33 0.10]);
        plot(ax1, data.t, data.wind_z, 'LineWidth', 1.5, 'Color', [0.12 0.62 0.30]);
        legend(ax1, 'wind_x', 'wind_y', 'wind_z', 'Location', 'best');
    else
        text(ax1, 0.5, 0.5, 'Aucune composante de vent disponible', 'Units', 'normalized', ...
            'HorizontalAlignment', 'center');
    end
    title(ax1, 'Evolution des composantes du vent');
    ylabel(ax1, 'Vitesse du vent (m/s)');

    ax2 = nexttile(tl, 2);
    hold(ax2, 'on'); grid(ax2, 'on'); box(ax2, 'on');
    if has_tau
        plot(ax2, data.t, data.tau_wind_x, 'LineWidth', 1.5, 'Color', [0.49 0.18 0.56]);
        plot(ax2, data.t, data.tau_wind_y, 'LineWidth', 1.5, 'Color', [0.93 0.69 0.13]);
        plot(ax2, data.t, data.tau_wind_z, 'LineWidth', 1.5, 'Color', [0.30 0.75 0.93]);
        legend(ax2, 'tau_{wind,x}', 'tau_{wind,y}', 'tau_{wind,z}', 'Location', 'best');
    else
        text(ax2, 0.5, 0.5, 'Aucun moment de vent disponible', 'Units', 'normalized', ...
            'HorizontalAlignment', 'center');
    end
    title(ax2, 'Moments perturbateurs dus au vent');
    xlabel(ax2, 'Temps (s)');
    ylabel(ax2, 'Moment (N.m)');

    sgtitle(tl, sprintf('Diagnostic vent - mode %s', cfg.mode_name));
    saveas(fig, output_path);
    close(fig);
end
